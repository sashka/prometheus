...
func TestMatches(t *testing.T) {
	...
	for _, mt := range matcherTests {
		res := MetricsForLabelMatchers(storage, mt.matchers...)
...
func TestFingerprintsForLabels(t *testing.T) {
	...
	for _, mt := range matcherTests {
		resfps := storage.FingerprintsForLabels(mt.pairs...)
...
func TestRetentionCutoff(t *testing.T) {
	...
	for f := range s.FingerprintsForLabels(metric.LabelPair{"job", "test"}) {
		fp = f
		break
	}
...
func TestDropMetrics(t *testing.T) {
	...
	fps := s.FingerprintsForLabels(metric.LabelPair{clientmodel.MetricNameLabel, "test"})
	if len(fps) != 2 {
	...
	fps2 := s.FingerprintsForLabels(metric.LabelPair{clientmodel.MetricNameLabel, "test"})
	if len(fps2) != 1 {
	...
	fps3 := s.FingerprintsForLabels(metric.LabelPair{clientmodel.MetricNameLabel, "test"})
	if len(fps3) != 0 {
